...
const IMAGES = {
  ...
  hero: 'https://images.unsplash.com/photo-1511632765486-a01980e01a18?q=80&w=2070&auto=format&fit=crop',
  ...
  activities: {
    apiUnggun: 'https://images.unsplash.com/photo-1503511740924-f3b1404edfa7?w=800',
    karaoke: 'https://images.unsplash.com/photo-1516280440502-861f5c66b8bd?w=800',
    games: 'https://images.unsplash.com/photo-1528605248644-14dd04022da1?w=800',
    makan: 'https://images.unsplash.com/photo-1523580494112-071d31154a48?w=800'
  }
};
...
const PAKET_EXPERIENCE = [
  ...
  },
  {
    ...
    image: 'https://images.unsplash.com/photo-1523580494112-071d31154a48?w=800'
  }
];
...
export default function Home() {
  ...
  useEffect(() => {
    ...
    const interval = setInterval(() => {
      setProgress((prev) => {
        if (prev >= 100) {
          setActiveSubSlide((t) => (t + 1) % 3); // 3 is the number of images per category
          return 0;
        }
        return prev + 1;
      });
  ...
  const handleTouchEnd = () => {
    ...
    if (diff > 50) {
      setActiveSubSlide((prev) => (prev + 1) % 3);
    } else if (diff < -50) {
      setActiveSubSlide((prev) => (prev - 1 + 3) % 3);
    }
...
Mohon informasi mengenai ketersediaan dan proposal harga. Terima kasih!`;
  ...
  return (
    <main className="min-h-screen bg-[#F9F9F7]">
      ...
      {/* ========== NAVBAR FIXED ========== */}
      <nav className="fixed top-0 w-full z-50 bg-[#163a28]/95 backdrop-blur-xl border-b border-white/20 shadow-lg">
      ...
      >
        ...
        <div className="max-w-7xl mx-auto relative z-10" onMouseEnter={() => setIsCarouselHovered(true)} onMouseLeave={() => setIsCarouselHovered(false)}>
          ...
          >
            ...
            <button
              onClick={() => setActiveSubSlide((prev) => (prev - 1 + 3) % 3)}
            ...
            <button
              onClick={() => setActiveSubSlide((prev) => (prev + 1) % 3)}
                ...
                {TARGET_MARKET.map((target, idx) => (
                  ...
                  >
                    ...
                    <div className="absolute inset-0 flex flex-col justify-end p-4 text-left">
                      <h4 className={`text-white font-serif font-bold transition-all duration-500 ${activeTarget === idx ? 'text-base md:text-lg text-[#98D8A0] translate-y-0' : 'text-xs md:text-sm translate-y-2'}`}>
                        {target.title}
                      </h4>
                    </div>
                  </div>
      ...
      <footer className="bg-[#112419] text-white pt-20 pb-8 px-4 border-t-4 border-[#163a28]">
        <div className="max-w-7xl mx-auto">
          <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-4 gap-12 mb-16">
            ...
            <div className="lg:col-span-1">
              <h4 className="font-sans font-bold text-sm text-white mb-6 uppercase tracking-widest">Kontak B2B</h4>
              <div className="space-y-5">
            ...
            <div className="lg:col-span-1">
              <div className="bg-white/5 border border-white/10 rounded-[1.5rem] p-6">
                <h4 className="font-serif font-bold text-xl text-white mb-3">Ikuti Kami</h4>
                <div className="flex gap-4">
                  <a href="https://www.instagram.com/vilakampunggunung?igsh=MXV6N21iMnBjcnhmMw==" target="_blank" rel="noopener noreferrer" className="h-10 w-10 bg-white/10 rounded-full flex items-center justify-center hover:bg-[#98D8A0] hover:text-[#112419] transition-colors">
                    <InstagramIcon className="h-4 w-4" />
                  </a>
                  <a href="https://www.tiktok.com/@villakampunggunung?_r=1&_t=ZS-96Syg9IYbjg" target="_blank" rel="noopener noreferrer" className="h-10 w-10 bg-white/10 rounded-full flex items-center justify-center hover:bg-[#98D8A0] hover:text-[#112419] transition-colors">
                    <TikTokIcon className="h-4 w-4" />
                  </a>
                  <a href="https://www.facebook.com/share/1Jt8mHA8cb/" target="_blank" rel="noopener noreferrer" className="h-10 w-10 bg-white/10 rounded-full flex items-center justify-center hover:bg-[#98D8A0] hover:text-[#112419] transition-colors">
